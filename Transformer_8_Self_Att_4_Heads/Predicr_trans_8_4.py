# -*- coding: utf-8 -*-
"""
Created on Tue Jan 17 00:44:14 2023
...
#from utils import TransformerModel, Model, Dataset, get_loader
...
# -*- coding: utf-8 -*-
"""
Created on Tue Jan 17 00:53:28 2023
...
#with open('token2ind.pkl', 'wb') as f:
...
label_counts = {}
for letter in proteins_test :
   #protein_seq = i
 # for letter in protein_seq:
#    letter =s.encode_as_pieces(letter)
    if letter in label_counts:
        label_counts[letter] += 1
    else:
        label_counts[letter] = 1

#print('label_counts jjjjjjjjjjjjjjjjjjjj',label_counts)

path_vocab = label_counts.keys()
token2ind_label = {} # the 4 first indices are reserved to special tokens
for idx, line in enumerate(path_vocab):
    word = line.split()[0].strip()
    token2ind_label[word] = idx  #fill me

ind2token_label = {v: k for k, v in token2ind_label.items()} #fill me
...
#load checkpoint
checkpoint = torch.load("Models/Test_light_model_1500.pt")
#load state dict
model.base.load_state_dict(checkpoint['model_state_dict'])


f= open("data/pred.sequences.spm","w+", encoding="utf-8")
...
f= open("data/pred.label","w+", encoding="utf-8")

for i in proteins_test:

    f.write(f'{token2ind_label[i]}'+' \n')
f.close()



path_data_pred = "data/pred.sequences.spm"
path_labels_pred = "data/pred.label"
...
for idx, data in enumerate(data_loader): #step 1
    ...
    with torch.no_grad() :
        ...
        output = output[-1]
        output = output.view(-1, output.shape[-1])
        max_prob_indice = output
    #max_prob_indice = output.argmax(-1) 

    target =  data[1] 
    target = target.to(device)

    predictions += [max_prob_indice]
    
print(predictions)

y_hat = torch.cat(predictions, dim=0)
with torch.no_grad() :
    s = nn.Softmax(dim=1)
    y_hat_proba = s(y_hat)
# Write predictions to a file
with open('Submissions/Trans_1500ep_submission.csv', 'w') as csvfile:
